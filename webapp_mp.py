...
def _run_job(job: Job):
    ...
    try:
        ...
        # so NN-over-members has a representative to match against. Up from
        # 120 — addressed remaining flicker on edge-pose frames where no
        # 120-sample member was close enough to the target embedding.
        max_samples = 200
        ...
        # 0.12 default: with NN-over-members (max sim across all cluster
        # member embeddings) the score floor is high enough that 0.12 still
        # rejects extras while catching profile / motion-blur / wide-shot
        # frames that were just below 0.15. Below 0.08 starts false-positiving
        # on lookalike extras.
